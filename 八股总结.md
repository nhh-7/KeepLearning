...
指针：指针是一个变量，其值为另一个变量的内存地址

- 指针可以被重新赋值，指向不同的对象
- 指针可以为 nullptr
- 指针可以进行算术运算

引用是对象的别名，它引用另一个已存在的对象

- 引用必须在声明时初始化
- 引用一旦初始化后，不能更改引用的对象
- 引用不能为 null

野指针：是指指向已被释放或无效内存区域的指针
...
- shared_ptr
  - 线程安全性
    - 引用计数是原子操作，安全
    - 多个线程同时对一个shared_ptr进行赋值或重置操作，不安全
- unique_ptr
- weak_ptr -- 为了解决shared_ptr的循环引用问题（两个对象相互持有对方的shared_ptr）

### 左值

- 有明确的存储位置（地址），可以取地址。
- 例子：变量名、返回左值引用的函数、`++i` 表达式。

### 右值

- 临时对象，没有持久的内存地址。
- 例子：字面量（如 `42`、`"hello"`）、运算表达式（如 `a+b`）、返回非引用类型的函数。

### 右值引用

- 右值引用通过`&&`符号声明，用于绑定到右值（临时对象、字面量等）
- 右值引用是对右值的 “具名引用”，也就是说右值引用的这个变量是可以取地址的，它让原本转瞬即逝的右值获得了可被长期操作的 “身份”，从而实现对临时资源的复用。
- 主要用途：
  - 移动语义
  - 完美转发
...
- 当`const`引用绑定右值时，编译器会隐式创建一个**临时对象**，将右值的值复制到临时对象中，然后让引用指向该临时对象
- 普通临时对象在表达式结束后销毁，但被`const`引用绑定后，其生命周期会延长至引用的作用域结束

### Lambda表达式的好处

- 代码简洁性：避免命名函数的冗余
- 上下文捕获：灵活访问外部变量
- 编译器可将 Lambda 表达式的代码直接嵌入调用处，避免函数调用开销。
- C++14后，Lambda表达式可以通过`auto`参数实现泛型，适配不同类型参数

### 新特性

C++17

- `std::optional`：表示可能不存在的值（替代空指针）。
- `std::variant`：类型安全的联合体，可存储多种类型中的一种。
- `std::any`：存储任意类型的值。
- 结构化绑定：直接将 tuple、pair、结构体或数组的成员拆分为独立变量。

C++20

- Concepts：约束模板参数的类型要求
- 协程：通过 `co_await`、`co_yield`、`co_return` 实现异步编程
- 模块：替代 `#include`，加速编译并封装实现细节
- 三路比较运算符

C++23

- std::expected：表示可能失败的结果（结合成功值与错误类型）
- 原子智能指针：提供线程安全的智能指针操作
...
- 模板特化
- 可变参数模板
- Type Traits-在编译器获取或操作 类型信息
- 模板元编程 - 在编译器计算或生成代码

### vector的n次push_back

加入vector初始容量为1，每次扩容为原来的两倍，那么需要进行logn次扩容，每次扩容的时间复杂度是扩容前vector的大小。总的时间复杂度就是扩容这么多次的复杂度之和再加上push n 个元素的复杂度。总共 2n -1 , 时间复杂度为O(n);
...
- push_back() -- 如果触发了扩容操作，所有迭代器失效
- insert() -- 插入点及之后的迭代器失效，如果扩容，所有都失效
- erase() -- 删除点及之后的迭代器失效

### static修饰普通非成员函数

- 使得该函数的作用域仅限于当前的源文件中
- 不能在其他源文件中访问或链接这个函数（即使是使用extern也不行）

### static修饰全局变量

- 该变量只能在定义它的源文件（`.cpp` 文件）中访问
- **不能被其他源文件使用**，即使使用 `extern` 关键字也无法访问
...
- 宏替换
- 头文件包含：将`#include`指定的文件内容插入到当前文件
- 条件编译：根据`#ifdef`、`#ifndef`、`#if`等指令选择性包含代码
...
- 重定位：修正目标文件中的临时地址，生成最终内存地址
- 库链接：
  - 静态链接：代码嵌入可执行文件，体积大但无需依赖
  - 动态链接：仅记录库的依赖关系，运行时加载，多个程序共享，节省内存
